...
! --- --------------------------------------------
  ...
  do j=G%jsc-1,G%jec+1 ; do i=G%isc-1,G%iec+1 ; if (G%mask2dT(i,j)>0.) then
    ...
    if (allocated(tv%SpV_avg)) then  ! This is the fully non-Boussinesq version
      ...
      enddo
      if (dz_tot <= CS%min_dilate*(G%bathyT(i,j)+G%meanSL(i,j))) then
        dilate = CS%min_dilate
      elseif (dz_tot >= CS%max_dilate*(G%bathyT(i,j)+G%meanSL(i,j))) then
        dilate = CS%max_dilate
      else
        dilate = dz_tot / (G%bathyT(i,j)+G%meanSL(i,j))
      endif
    else
      nominalDepth = (G%bathyT(i,j)+G%meanSL(i,j))*GV%Z_to_H
