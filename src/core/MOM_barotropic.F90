...
type, public :: barotropic_CS ; private
  ...
  integer :: id_pfubt_hifreq = -1, id_pfvbt_hifreq = -1, id_cfubt_hifreq = -1, id_cfvbt_hifreq = -1
  integer :: id_hcubt_hifreq = -1, id_hcvbt_hifreq = -1
...
!$OMP end parallel
  ...
  do n=1,nstep+nfilter
    ...
    if (CS%hydraulic_control_vel) then
      do j=jsv,jev ; do I=isv-1,iev
        if (ubt(I,j)>=0.0) then
          ubt(I,j) = min(ubt(I,j), CS%hc_coef * max(eta(i,j)+G%bathyT(i,j), 0.0)**0.5)
        else
          ubt(I,j) = max(ubt(I,j), -CS%hc_coef * max(eta(i+1,j)+G%bathyT(i+1,j), 0.0)**0.5)
        endif
      enddo ; enddo

      do J=jsv-1,jev ; do i=isv,iev
        if (vbt(i,J)>=0.0) then
          vbt(i,J) = min(vbt(i,J), CS%hc_coef * max(eta(i,j)+G%bathyT(i,j), 0.0)**0.5)
        else
          vbt(i,J) = max(vbt(i,J), -CS%hc_coef * max(eta(i,j+1)+G%bathyT(i,j+1), 0.0)**0.5)
        endif
      enddo ; enddo
    endif
    ...
    if (MOD(n+G%first_direction,2)==1) then
      ...
      if (CS%hydraulic_control_pf) then
        do J=jsv-1,jev ; do i=isv,iev
          if (vbt(i,J)>=0.0) then
            PFv_hc(i,J) = min(0.0, CS%hc_coef * max(eta(i,j)+G%bathyT(i,j), 0.0)**0.5 - vbt(i,J))/dtbt
          else
            PFv_hc(i,J) = max(0.0, -CS%hc_coef * max(eta(i,j+1)+G%bathyT(i,j+1), 0.0)**0.5 - vbt(i,J))/dtbt
          endif
        enddo ; enddo
      ...
      if (CS%hydraulic_control_trans) then
        do J=jsv-1,jev ; do i=isv,iev
          if (vhbt(i,J)>=0.0) then
            vhbt(i,J) = min(vhbt(i,J), G%dx_Cv(i,J)*CS%hc_coef * max(eta(i,j)+G%bathyT(i,j), 0.0)**1.5)
          else
            vhbt(i,J) = max(vhbt(i,J), -G%dx_Cv(i,J)*CS%hc_coef * max(eta(i,j+1)+G%bathyT(i,j+1), 0.0)**1.5)
          endif
        enddo ; enddo
      endif
      ...
      if (CS%hydraulic_control_pf) then
        do j=jsv,jev ; do I=isv-1,iev
          if (ubt(I,j)>=0.0) then
            PFu_hc(I,j) = min(0.0, CS%hc_coef * max(eta(i,j)+G%bathyT(i,j), 0.0)**0.5 - ubt(I,j))/dtbt
          else
            PFu_hc(I,j) = max(0.0, -CS%hc_coef * max(eta(i+1,j)+G%bathyT(i+1,j), 0.0)**0.5 - ubt(I,j))/dtbt
          endif
        enddo ; enddo
      elseif (CS%hydraulic_control_pfv2) then
        do j=jsv,jev ; do I=isv-1,iev
      ...
      if (CS%hydraulic_control_trans) then
        do j=jsv,jev ; do I=isv-1,iev
          if (uhbt(I,j)>=0.0) then
            uhbt(I,j) = min(uhbt(I,j), G%dy_Cu(I,j)*CS%hc_coef * max(eta(i,j)+G%bathyT(i,j), 0.0)**1.5)
          else
            uhbt(I,j) = max(uhbt(I,j), -G%dy_Cu(I,j)*CS%hc_coef * max(eta(i+1,j)+G%bathyT(i+1,j), 0.0)**1.5)
          endif
        enddo ; enddo
      endif
    else
    ...
    if (CS%debug_bt) then
      ...
      call hchksum(PFv_hc, trim(mesg)//" PF_hcv", CS%debug_BT_HI, haloshift=iev-ie,scale=US%L_T_to_m_s*US%s_to_T)
      call hchksum(PFu_hc, trim(mesg)//" PF_hcu", CS%debug_BT_HI, haloshift=iev-ie,scale=US%L_T_to_m_s*US%s_to_T)
    ...
    if (do_hifreq_output) then
      ...
      if (CS%id_hcubt_hifreq > 0) call post_data(CS%id_hcubt_hifreq, PFu_hc(IsdB:IedB,jsd:jed), CS%diag)
      if (CS%id_hcvbt_hifreq > 0) call post_data(CS%id_hcvbt_hifreq, PFv_hc(isd:ied,JsdB:JedB), CS%diag)
      if (CS%id_cfubt_hifreq > 0) call post_data(CS%id_cfubt_hifreq, Cor_u(IsdB:IedB,jsd:jed), CS%diag)
      if (CS%id_cfvbt_hifreq > 0) call post_data(CS%id_cfvbt_hifreq, Cor_v(isd:ied,JsdB:JedB), CS%diag)
...
# include "version_variable.h"
  ...
  call get_param(param_file, mdl, "USE_HYDRAULIC_CONTROL", CS%hydraulic_control_vel, &
