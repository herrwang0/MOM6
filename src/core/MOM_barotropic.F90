  ...
  do n=1,nstep+nfilter
    ...
    do j=js,je ; do i=is,ie
      if ((i + G%HI%idg_offset == i_tgt) .and. (j + G%HI%jdg_offset == j_tgt)) then
        ...
        write(mesg, *) 'DEBUG step', n, 'visc_rem_v(i+1,J)', visc_rem_v(i+1,J,1)
        call MOM_error(WARNING, trim(mesg), all_print=.true.)

        write(mesg, *) 'DEBUG step', n, 'v(i+1,J), vhbt(i+1,J)', vbt_trans(i+1,J), &
        ...
        call MOM_error(WARNING, trim(mesg), all_print=.true.)
        write(mesg, *) 'DEBUG step', n, 'BTCL_v(i+1,J)', &
    ...
    do j=js,je ; do i=is,ie
      if ((i + G%HI%idg_offset == i_tgt) .and. (j + G%HI%jdg_offset == j_tgt)) then
        ...
        call MOM_error(WARNING, trim(mesg), all_print=.true.)
        write(mesg, *) 'DEBUG step', n, 'eta_in(i+1), eta_src(i+1)', eta_in(i+1,j), eta_src(i+1,j)
        call MOM_error(WARNING, trim(mesg), all_print=.true.)
