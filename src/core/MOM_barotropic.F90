...
# include "version_variable.h"
  ...
  if (.not.CS%nonlin_stress) then
    Z_to_H = GV%Z_to_H ; if (.not.GV%Boussinesq) Z_to_H = GV%RZ_to_H * CS%Rho_BT_lin
    do j=js,je ; do I=is-1,ie
      htot = max(G%bathyT(i+1,j) + G%meanSL(i+1,j), 0.0) + max(G%bathyT(i,j) + G%meanSL(i,j), 0.0)
    ...
    enddo ; enddo
    do J=js-1,je ; do i=is,ie
      htot = max(G%bathyT(i,j+1) + G%meanSL(i,j+1), 0.0) + max(G%bathyT(i,j) + G%meanSL(i,j), 0.0)
