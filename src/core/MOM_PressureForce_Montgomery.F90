...
subroutine PressureForce_Mont_Bouss(h, tv, PFu, PFv, G, GV, US, CS, p_atm, pbce, eta)
  ...
  if (CS%calculate_SAL) then
    ...
    !$OMP parallel do default(shared)
    do j=Jsq,Jeq+1
      do i=Isq,Ieq+1 ; SSH(i,j) = -G%meanThick(i,j) ; enddo
    ...
    if (present(rho_star)) then
     !$OMP parallel do default(shared) private(Ihtot)
      ...
      do j=Jsq,Jeq+1
        do i=Isq,Ieq+1
          Ihtot(i) = GV%H_to_Z / ((e(i,j,1)-e(i,j,nz+1)) + dz_neglect)
          press(i) = -Rho0xG*(e(i,j,1) - (G%meanThick(i,j) - G%bathyT(i,j)))
        enddo
        ...
        do k=2,nz
          do i=Isq,Ieq+1
            press(i) = -Rho0xG*(e(i,j,K) - (G%meanThick(i,j) - G%bathyT(i,j)))
