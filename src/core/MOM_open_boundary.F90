...
!>@}
character(len=8), parameter :: PHYS_FIELD_NAMES(NUM_PHYS_FIELDS) = &  !< Physical field name strings used by
    [character(len=8) :: 'U', 'V', 'DVDX', 'DUDY', 'SSH', 'Uamp', &   !! input parameter
     'Uphase', 'Vamp', 'Vphase', 'SSHamp', 'SSHphase', 'TEMP', 'SALT']
...
subroutine segment_determine_required_fields(segment, tides, temp_salt)
  ...
  if (segment%nudged_grad .and. segment%specified_grad) &
...
subroutine allocate_segment_field_data(field, OBC, segment, US, inputdir, filename, varname, &
  ...
  type(unit_scale_type),  intent(in)    :: US          !< A dimensional unit scaling type
  character(len=*), &
                          intent(in)    :: inputdir, & !< The directory of input files
                                           filename, & !< Input file name
                                           varname,  & !< Variable name in the input file
                                           suffix      !< Variable name suffix, "_segment_xxx"
  ...
  if (trim(filename) /= 'none') then
    ...
    if ((field%nk_src > 1) .and. (.not. field_is_tidal(field%name))) then ! nk_src is depth
      ! Check if "dz_$varname" dimension matches with "$varname"
...
subroutine initialize_segment_data(GV, US, OBC, PF, turns)
  ...
  do n=1,OBC%number_of_segments
    ...
    call segment_determine_required_fields(segment, &
                                           tides=OBC%add_tide_constituents, temp_salt=.false.)
    ...
    do m = NUM_PHYS_FIELDS+1, segment%num_fields
      ...
      do mm=1,num_manifest_fields ; if (trim(bgc_input) == trim(phys_inputs(mm))) then
        write(mesg,'("Input parameter for OBC segment ",I0," contains an BGC tracer.")') &
              n, trim(bgc_input)
        call MOM_error(FATAL, trim(routine_name) // ", " // trim(mesg))
      endif ; enddo
