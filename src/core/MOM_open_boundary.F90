...
subroutine register_segment_tracer(tr_ptr, ntr_index, param_file, GV, segment, &
  ...
  ! Assign per-tracer length scale inverses
  if ((segment%tr_Reg%Tr(ntseg)%resrv_lfac_in  == -1.0) &
      .and. (segment%Tr_InvLscale_in  == -1.0)) then
  ...
  if ((segment%tr_Reg%Tr(ntseg)%resrv_lfac_out == -1.0) &
      .and. (segment%Tr_InvLscale_out == -1.0)) then
