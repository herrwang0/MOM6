...
subroutine step_MOM_tracer_dyn(CS, G, GV, US, h, Time_local)
  ...
  ! Tracer-step OBC path: active when locked (BGC locked to DT_TRACER) or when the bug fix is
  ! enabled (OBC_TRACER_DZ_BUG=F). OBC_TRACER_DZ_BUG controls whether dz is recomputed from the
  ! current layer thicknesses before reading.
  if (associated(CS%OBC)) then
    if (CS%OBC%lock_bgc_obc_period .or. (.not. CS%OBC%tracer_dz_bug)) then
