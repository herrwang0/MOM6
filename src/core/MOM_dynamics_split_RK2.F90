...
type, public :: MOM_dyn_split_RK2_CS ; private
  ...
  integer :: id_vremu    = -1, id_vremv    = -1
  integer :: id_vremup    = -1, id_vremvp    = -1
...
! u_bc_accel = CAu + PFu + diffu(u[n-1])
  ...
  do k=1,nz
    do j=js,je ; do I=Isq,Ieq
      up(I,j,k) = G%mask2dCu(I,j) * (u_inst(I,j,k) + dt_pred_test * u_bc_accel(I,j,k))
    enddo ; enddo
    do J=Jsq,Jeq ; do i=is,ie
      vp(i,J,k) = G%mask2dCv(i,J) * (v_inst(i,J,k) + dt_pred_test * v_bc_accel(i,J,k))
    enddo ; enddo
  enddo
  ...
  call thickness_to_dz(h, tv, dz, G, GV, US, halo_size=1)
  call vertvisc_coef(up, vp, h, dz, forces, visc, tv, dt_pred_test, G, GV, US, CS%vertvisc_CSp, CS%OBC, VarMix)
  call vertvisc_remnant(visc, CS%visc_rem_u, CS%visc_rem_v, dt_pred_test, G, GV, US, CS%vertvisc_CSp)
...
! u_accel_bt = layer accelerations due to barotropic solver
  if (associated(CS%BT_cont) .or. CS%BT_use_layer_fluxes) then
    ...
    call continuity(u_inst, v_inst, h, hp, uh_in, vh_in, dt_pred_test, G, GV, US, CS%continuity_CSp, CS%OBC, pbv, &
  ...
  call btstep(u_inst, v_inst, eta, dt_pred_test, u_bc_accel, v_bc_accel, forces, CS%pbce, CS%eta_PF, u_av, v_av, &
...
! u_av  <- u_av  + dt_pred d/dz visc d/dz u_av
  ...
  ! hp = h + dt * div . uh
  call cpu_clock_begin(id_clock_continuity)
  call continuity(up, vp, h, hp, uh, vh, dt_pred_test, G, GV, US, CS%continuity_CSp, CS%OBC, pbv, &
...
! u_bc_accel = CAu + PFu + diffu(u[n-1])
  ...
  ! call thickness_to_dz(h, tv, dz, G, GV, US, halo_size=1)
  ! call vertvisc_coef(u_inst, v_inst, h, dz, forces, visc, tv, dt_pred, G, GV, US, CS%vertvisc_CSp, CS%OBC, VarMix)
  ! call vertvisc_remnant(visc, CS%visc_rem_u, CS%visc_rem_v, dt_pred, G, GV, US, CS%vertvisc_CSp)
  ! if (CS%id_vremup > 0) call post_data(CS%id_vremup, CS%visc_rem_u, CS%diag)
  ! if (CS%id_vremvp > 0) call post_data(CS%id_vremvp, CS%visc_rem_v, CS%diag)
  ...
  call thickness_to_dz(h, tv, dz, G, GV, US, halo_size=1)
  ! coefficients are not changed by vertvisc_coef ?
...
! h_av = (h_in + h_out)/2 . Going in to this line, h_av = h_in.
  ...
  if (CS%test_bt_visc_rem) then
    if (CS%id_vremu > 0) call post_data(CS%id_vremu, CS%visc_rem_u, CS%diag)
    if (CS%id_vremv > 0) call post_data(CS%id_vremv, CS%visc_rem_v, CS%diag)
  endif
...
!  Accel_diag%u_av => CS%u_av ; Accel_diag%v_av => CS%v_av
  ...
  CS%id_vremu = register_diag_field('ocean_model', 'visc_rem_u_mom', diag%axesCuL, Time, &
      'Zonal Coriolis and Advective Acceleration', 'm s-2', conversion=US%L_T2_to_m_s2)
  CS%id_vremv = register_diag_field('ocean_model', 'visc_rem_v_mom', diag%axesCvL, Time, &
      'Meridional Coriolis and Advective Acceleration', 'm s-2', conversion=US%L_T2_to_m_s2)

  CS%id_vremup = register_diag_field('ocean_model', 'visc_rem_u_momp', diag%axesCuL, Time, &
      'Zonal Coriolis and Advective Acceleration', 'm s-2', conversion=US%L_T2_to_m_s2)
  CS%id_vremvp = register_diag_field('ocean_model', 'visc_rem_v_momp', diag%axesCvL, Time, &
      'Meridional Coriolis and Advective Acceleration', 'm s-2', conversion=US%L_T2_to_m_s2)
