...
! u_av  <- u_av  + dt_pred d/dz visc d/dz u_av
  ...
  if (associated(CS%OBC)) then
    ...
    call chksum_OBC_segments(CS%OBC, G, GV, US, nk=3, mesg='Pre-rad', do_dynamics=.false., do_tracer=.true.)
    call radiation_open_bdry_conds(CS%OBC, u_av, u_old_rad_OBC, v_av, v_old_rad_OBC, G, GV, US, dt_pred)
    call chksum_OBC_segments(CS%OBC, G, GV, US, nk=3, mesg='Post-rad', do_dynamics=.false., do_tracer=.true.)
