...
type, public :: hor_visc_CS ; private
  ...
  logical :: decomp_ke       !< If true, kinetic energy source from horizontal viscosity is decomposed into
                             !! a flux term and a squared term. Different from `KE_horvisc` in MOM_diagnostics,
                             !! the KE terms is calculated using velcosity (u, v), rather than transport (uh, vh).
...
subroutine horizontal_viscosity(u, v, h, diffu, diffv, MEKE, VarMix, G, GV, US, &
  ...
  ! term. The sign-definiteness is not guaranteed (yet) for Biharmonic viscosity, as it is not
  ! the case for the continous form, which the implementation we have here is based on.
...
# include "version_variable.h"
  ...
  if (CS%decomp_ke) then
      ...
      CS%id_KE_Lap_div = register_diag_field('ocean_model', 'KE_horvisc_Lap_div', diag%axesTL, Time, &
        'Divergence of the flux term from decomposing KE due to Laplacian horizontal viscosity', &
        'm2 s-3', conversion=(US%L_T_to_m_s**2)*US%s_to_T)
      ...
      CS%id_KE_bih_div1 = register_diag_field('ocean_model', 'KE_horvisc_bih_div1', diag%axesTL, Time, &
        'Divergence of the first flux term from decomposing KE due to Biharmonic horizontal viscosity', &
        'm2 s-3', conversion=(US%L_T_to_m_s**2)*US%s_to_T)
