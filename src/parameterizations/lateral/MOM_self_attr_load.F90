...
# include "version_variable.h"
  ...
  if (CS%use_bpa) then
    ...
    call get_param(param_file, mdl, "SAL_PBOT_REF_CONFIG", bpa_config, &
                   "A string that determines how the reference bottom pressure for SAL "//&
                   "is specified: \n "//&
                   " \t init - calculated by thickness, temperature and salinity \n"//&
                   " \t\t from initialiaztion and assuming surface pressure is zero. \n"//&
                   " \t file (default) - read from the file specified by (REF_BOT_PRES_FILE)", &
                   default="file")
    select case (trim(lowercase(bpa_config)))
      case ("file")
        ...
        call get_param(param_file, mdl, "REF_BOT_PRES_VARNAME", pbot_ref_varname, &
                       ...
                       default="pbot")
        filename = trim(slasher(inputdir))//trim(pbot_ref_file)
