...
subroutine vertvisc(u, v, h, forces, visc, dt, OBC, ADp, CDp, G, GV, US, CS, &
  ...
  do j=G%jsc,G%jec
    ...
    !
    do I=Isq,Ieq ; if (do_i(I)) then
      b_denom_1 = CS%h_u(I,j,1) + dt * (Ray(I,1) + Ray_lin(I,1) + CS%a_u(I,j,1))
    ...
    do k=2,nz ; do I=Isq,Ieq ; if (do_i(I)) then
      ...
      b_denom_1 = CS%h_u(I,j,k) + dt * (Ray(I,k) + Ray_lin(I,k) + CS%a_u(I,j,K)*d1(I))
  ...
  do J=Jsq,Jeq
    ...
    do i=is,ie ; if (do_i(i)) then
      b_denom_1 = CS%h_v(i,J,1) + dt * (Ray(i,1) + Ray_lin(i,1) + CS%a_v(i,J,1))
    ...
    do k=2,nz ; do i=is,ie ; if (do_i(i)) then
      ...
      b_denom_1 = CS%h_v(i,J,k) + dt * (Ray(i,k) + Ray_lin(i,k) + CS%a_v(i,J,K)*d1(i))
